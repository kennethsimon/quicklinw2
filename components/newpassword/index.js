...
const Newpassword = ({navigation, route}) => {
  ...
  const {state, newpassword} = useContext(AuthContext);
  ...
  const onSubmit = () => {
    ...
    if (validatePassword(password)) {
      newpassword({email_address: params.data.email_address, password});
    } else {
      setError(
        ...
        <View style={styles.contentcontainer}>
          ...
          <View style={styles.form}>
            ...
            <Button
              ...
              isLoading={state.loading === 'newpassword'}
              isDisabled={state.loadin === 'newpassword'}
              size={'lg'}
              bgColor={'#539D8B'}
              mt={'3'}>
              Submit
